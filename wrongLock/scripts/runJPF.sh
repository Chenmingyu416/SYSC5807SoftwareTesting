...
subject_dir=${experiment_root}/wrongLock
...
then
    ...
    then
        echo Invoking JPF
        ${jpf_root}/bin/jpf +classpath=${subject_dir}/source Main \
            1 1 2>&1 | tee ${subject_dir}/outputs/wrongLockJPF.log
    else 
        if test $2 = "random" 
        then
            echo Invoking JPF
		  ...
		  #expect -c "set timeout $TIMEOUT; set log_file ${subject_dir}/outputs/wrongLockJPF${seed}.log; spawn \
                  #${jpf_root}/bin/jpf +classpath=${subject_dir}/source \
                  #+cg.randomize_choices=path +cg.seed=${seed} \
                  #Main 1 1 2>&1 |; log_file ${subject_dir}/outputs/wrongLockJPF${seed}.log; expect; log_file" \
                  #run JPF without expect
                  #comment out the following 4 lines if using expect above
                ${jpf_root}/bin/jpf +classpath=${subject_dir}/source \
                    +cg.randomize_choices=path +cg.seed=$seed \
                    Main 1 1 2>&1 | \
                    tee ${subject_dir}/outputs/wrongLockJPF$seed.log 
                X=$((X+1))
